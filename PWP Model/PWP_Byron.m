function PWP_Byron(met_input_file, profile_input_file, pwp_output_file)
%--------------------------------------------------------------------------
...
% Updated Feb 3, 2025 by Jaime Palter to replace sw_ functions with gsw_
%--------------------------------------------------------------------------
% input (.mat format)
...
d	= gsw_pot_rho_t_exact(s(:,1),t(:,1),0,0);
...
pwp_output.d = gsw_pot_rho_t_exact(pwp_output.s,pwp_output.t,0,0);
...
t(1:j) = mean(t(1:j));
s(1:j) = mean(s(1:j));
d(1:j) = gsw_pot_rho_t_exact(s(1:j),t(1:j),0,0);
u(1:j) = mean(u(1:j));
v(1:j) = mean(v(1:j));
end % mix5
...
s(j+1)		= s(j+1)-ds;
s(j) 			= s(j)+ds;
d(j:j+1)	= gsw_pot_rho_t_exact(s(j:j+1),t(j:j+1),0,0);
du				= (u(j+1)-u(j))*f/2;
u(j+1)		= u(j+1)-du;
u(j) 			= u(j)+du;
dv				= (v(j+1)-v(j))*f/2;
